...
export default function CourseWorkList({ id }: { id: string }) {
  ...
  const { data, isLoading, isError, isSuccess } = useCourseWork(id)
          ...
          <Table>
            ...
            {allSubmissions.length === 0 ? (
              ...
              allSubmissions.map((submission: any) => {
                ...
                let updateTimeDisplay = 'N/A'
                if (submission.updateTime) {
                  const updateTime = new Date(submission.updateTime)
                  if (!isNaN(updateTime.getTime())) {
                    updateTimeDisplay = format(updateTime, 'PP p')
                  } else {
                    console.warn('Invalid date encountered:', submission.updateTime)
                  }
                }
                ...
                return (
                  <TableRow key={submission.id}>
                    ...
                    <TableCell>{updateTimeDisplay}</TableCell>
                    <TableCell>
